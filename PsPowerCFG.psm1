...
function Export-SystemSleepDiagnostics {
    <#
    .SYNOPSIS
    ...
    param (
        [Parameter(Mandatory = $false, ParameterSetName = 'OutputReport')]
        [System.IO.FileInfo]$OutFile = "$($PSScriptRoot)\system-sleep-diagnostics.html",
        [Parameter(Mandatory = $false, ParameterSetName = 'OutputReport')]
        [switch]$FormatXML,
        [Parameter(Mandatory = $false, ParameterSetName = 'OutputReport')]
        [int]$Duration,
    ...
    process {
        If ($FormatXML) {
            ...
            Start-Process -FilePath powercfg.exe -ArgumentList "/SYSTEMSLEEPDIAGNOSTICS /OUTPUT ""$($OutFile)"" /XML" -WindowStyle Hidden -Wait
            Write-Output "Sleep Study report saved to file path: $($OutFile)"
            Return
        }
        ...
        If ($ReformatXML) {
            If (!(Test-Path -Path "$($FilePath)" -ErrorAction SilentlyContinue)) {
                ...
                Start-Process -FilePath powercfg.exe -ArgumentList "/SYSTEMSLEEPDIAGNOSTICS /TRANSFORMXML ""$($FilePath)""" -WindowStyle Hidden -Wait -WorkingDirectory "$(Split-Path -Path $FilePath -Parent)"
                Write-Output "Sleep Study report saved to file path: $(Split-Path -Path $FilePath -Parent)\sleepstudy-report.html"
                Return
            }
        }
        ...
        Write-Verbose "Formatting Report as HTML"
        Start-Process -FilePath powercfg.exe -ArgumentList "/SYSTEMSLEEPDIAGNOSTICS /OUTPUT ""$($OutFile)""" -WindowStyle Hidden -Wait
        Write-Output "Sleep Study report saved to file path: $($OutFile)"
    }
}
...
function Export-SRUMDB {
    ...
    process {
        If ($FormatXML) {
            ...
            Start-Process -FilePath powercfg.exe -ArgumentList "/SRUMUTIL /OUTPUT ""$($OutFile)"" /XML" -WindowStyle Hidden -Wait -RedirectStandardError "$($env:temp)\error.txt" -RedirectStandardOutput "$($env:temp)\out.txt"
            $errorContent = Get-Content "$($env:temp)\error.txt"
            $outputContent = Get-Content "$($env:temp)\out.txt"

            Write-Output "StdErr: $($errorContent)"
            Write-Output "StdOut: $($outputContent)"
        }
    }
}